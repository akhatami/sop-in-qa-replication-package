...
df = df[(df['maven'] == '1') & (df['gradle'] != '1') & (df['android'] != '1') & (df['maven_usage_on_root'] == '1')]
...
for index, row in df.iterrows():
    ...
    try:
        ...
        print_time()
        # 60 minutes limit
        command = "cd {0}; mvn clean package -DskipTests --batch-mode -e; echo '**DONE-EXECUTING**' ".format(repo_path)
        ...
        if "BUILD SUCCESS" in output.stdout:
            log_path += "successful"
            print("1!")
        elif "BUILD FAILURE" in output.stdout:
            log_path += "failed"
            print("0!")
        else:
            log_path += "unclear"
